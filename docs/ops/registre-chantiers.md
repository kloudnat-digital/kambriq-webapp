...
| `A32`             | `EN COURS`          | CI Gate refuses while develop's latest journeys verdict is red; the label `merge-on-red-develop` plus a re-run releases it; an unreadable develop refuses. Throwaway #129: red refused, label released; green pending. Does not catch the first breaking merge. Cost: none                                                      |
...
| `I18`             | `EN COURS`          | one definition of the roles: the literal ban now scans `apps/web` and the e2e suite; 8 codes in 14 web files moved to `RoleCode`. The ROOT gate on `admin/kbs/layout.tsx` is reported and left untouched pending a decision. Pending: merge through the gate. Cost: none                                                        |
