...
| `G19`                         | `EN COURS`          | decided by Visquis on 26 September and applied: `Land.totalPrice` (renamed from `price`) is the source of truth, and `Land.pricePerM2` is generated by the database from it, so the two cannot drift; nothing multiplies by the surface anymore. Pending: the purchase page and land panel read on dev                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `confirmRemainingPayment`     | `A DECIDER`         | step 4 records the balance on the reservation alone. Closing it needs two decisions: the balance amount is `price - deposit`, which depends on G19's unit, and a payment must say whether it is the deposit or the balance (no such column), or a validated balance satisfies the deposit gate. No historical rows on dev: 0 of 200 reservations have a confirmed balance                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
### confirmRemainingPayment - the balance on the ledger - `A DECIDER`
...
### G19 - a 170 000 deposit beside a 1 631 830 000 balance - `EN COURS`
