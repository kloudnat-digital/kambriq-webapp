...
| `P10`                         | `EN COURS`          | the agent-page promises of an exclusive catalogue: two replaced with the copy Visquis validated on 26 September (#200); the last two (`products.kbs.advantages.network`, `products.kamnet.agentJourney.step5`) have proposed copy in an open PR, **not merged, for his approval**. The pin now reads both product pages, not named keys - #200's version named the three strings the row named and missed a third on the same page. Still open: the LANDS page rewrite (#174, held for copy)                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `confirmRemainingPayment`     | `A DECIDER`         | G20. The amount is settled since G19: `balanceFor(totalPrice)` = total minus `depositFor`, pinned to add up to the total exactly. What remains is Visquis's: a purpose on `Payment` (deposit or balance), written out in the entry - column, constraint, migration, and what it does to the triggers. No payment is created for the balance until then                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
