...
| `I19`                         | `EN COURS`          | no user without a role - fixed in code (#132). Read on dev, 26 September: **dev holds 8 role rows of 11** (`STAFF_VERIFY`, `STAFF_VALUATION`, `PARTNER_GEO` missing - the seed defect this fixed, which only a seed run brings to dev) and **2 users with no role**, both throwaway addresses created 4 September, before the fix. Pending: Visquis's call on the seed run or a targeted role upsert                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| Land price integer money      | `EN COURS`          | `Land.totalPrice` and the price history move from Float to BigInt (whole XAF), the last land price held as a floating-point number; the response envelope turns a BigInt into an exact number. Pending: the migration read back on dev                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
### Land price integer money - `EN COURS`
...
**Pending:** the migration read back on dev - the column types, and a land
answering with the same figures.
