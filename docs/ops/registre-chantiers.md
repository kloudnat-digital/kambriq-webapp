...
| `G8`             | `ARRETE`            | stopped twice at Part 0. **The re-land still has not happened**: `230b827` is not an ancestor of develop, and no PR carries it                      |
...
| `G8` blocker     | `A FAIRE`           | re-land `230b827` on develop (zero conflicts), deploy, then re-run G8. Until then dev emails every channel's coordinates to whoever clicks          |
...
**zero conflicts**. A branch from `230b827` and a PR to develop re-lands it.
