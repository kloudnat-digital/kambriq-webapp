# The 20 September wave - a frozen working note

> **This is not the record.** The record is
> [`docs/ops/registre-chantiers.md`](../registre-chantiers.md), and every
> chantier below now has an entry there: `I38`, `I42`, `I17`, `P9`, `P20`,
> `P21`, `A38`, `P11`, `A40`, `P2`, `P4`, `A45`, `A41` and `A46`.
>
> **It is frozen as of 25 September 2026 and nothing further is written here.**
>
> It was first frozen on 24 September, on the remediation branch, and the freeze
> did not reach develop: this file went on being written there as
> `WAVE_STATUS.md` for another day, gaining `A40`, `P2`, `P4`, `A45`, `A41` and
> `A46` - five hundred and fifty lines. The merge of 25 September brought them
> here, where the rename had already moved the file. **That is the same defect
> the freeze exists to end, and it ended only when the branches met**: a freeze
> written on one branch is a claim about a file, not a property of it, until
> every branch carrying that file has seen it.
> It sat at the repository root for five days as a second record, and for those
> five days the register still read _"Last closed: Friday 4 September"_ and
> carried none of this - so the file every session is told to trust was the
> stale one, and a reader who followed the instructions got the wrong answer.
>
> It is kept rather than deleted because it carries the working detail a
> register entry condenses: the mutation tables, the premises checked one by
> one, the copy approved key by key. **A note like this is worth keeping and is
> not worth trusting**: it was true when it was written and nothing updates it.
> Read it for how something was proved; read the register for what is true now.
