...
### Q1 — `CERTIFIED` meant nothing had been issued — `EN COURS`

**Settled, not deferred.** Reading the code answers it: `issueCertificate` is an
explicit admin route, `KbsCertificate` carries an `issuedBy` column, and revoking
takes a mandatory reason. That is a document somebody stands behind, and nothing
suggests issuance was ever meant to be automatic. So the missing certificate is
not the defect. **The status is.**

`gradeExam` set `CERTIFIED` and `certifiedAt` on the score alone. The candidate
was certified with no certificate, no `kcaNumber` and nobody's name against it.
`GET /kbs/certificate/me` answered `{"data":null}` to somebody the API called
certified.

**Somebody had already met this and patched around it.** `isCertified()` reads:
...
A status that needs a second check to be believed is not a status. That comment
is the defect, written down, at one call site, and left there.

**The part that was not cosmetic.** Grading also enqueued `GRANT_KCA_ROLE`, and
`KCA_CERTIFIED` gates the KAMNET agent routes
(`@Roles(RoleCode.KCA_CERTIFIED)`). Passing the exam made somebody a KAMNET
agent — before any certificate existed and before any human had approved it. **An
authorisation must not precede the credential it represents.**

**Decision, implemented.** `EXAM_PASSED` is what passing an exam earns;
`CERTIFIED` is what issuing a certificate confers.

| Before                                    | After                                          |
| ----------------------------------------- | ---------------------------------------------- |
| `gradeExam` → `CERTIFIED` + `certifiedAt` | `gradeExam` → `EXAM_PASSED`, no date           |
| grading grants `KCA_CERTIFIED`            | issuance grants it, with `issuedBy`            |
| `EXAM_PENDING → CERTIFIED` was legal      | `EXAM_PENDING → EXAM_PASSED → CERTIFIED`       |
| issuance set the status "if not already"  | issuance sets the status **and** `certifiedAt` |

**A third defect fell out of reading it.** The issuance precondition checked
`kbsCandidateProgress` — which records **module quizzes, not the exam**. One
passed quiz satisfied "has passed exam", so a candidate who had answered ten
questions about module 1 and never sat the certification exam could be issued a
KCA certificate. It now checks `kbsExam` with `status: PASSED`.

`eligibility` answers `EXAM_PASSED` explicitly ("examen réussi, certificat en
cours d'émission") instead of falling through, and `me/overview` returns
`nextAction: 'awaiting-certificate'`, so the candidate screen states it rather
than inferring it from a null.

**Proof:** five mutations, all firing — grading certifying again; the role
granted on the score again; issuance forgetting `certifiedAt`; a module quiz
counting as the exam again; `EXAM_PENDING → CERTIFIED` made legal again.
`EN COURS` until dev shows `EXAM_PASSED` after a pass and `CERTIFIED` only after
issuance. **Cost: none.** No migration — the column is a `String`.

**Left alone deliberately:** revocation returns a candidate to `EXAM_PENDING`,
forcing a resit. That is a policy question, not an inconsistency, and not mine to
change. `generateKcaNumber`'s comment still says _sequential per day_ while it
emits a random suffix (`KCA-20260904-N4OY`) — harmless until somebody sorts or
parses them.
