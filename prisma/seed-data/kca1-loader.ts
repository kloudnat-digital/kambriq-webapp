/**
 * KCA1 loader - the parsing half, with no IO and no side effects.
 *
 * `prisma/load-kca1.ts` is the script: it reads the docx off the Drive through
 * mammoth, reads the four quiz CSVs, calls into here, and writes the generated
 * seed data. Everything in this file takes strings and returns data, which is
 * what makes it testable without a deployment, a Drive mount, or a binary.
 *
 * Why the structure is detected from TEXT and never from markup: a fixture
 * generated by one tool and a document authored by another do not produce the
 * same HTML. The first fixture attempt emitted bare `<p>PARCOURS 0 - ...</p>`
 * where the real document emits `<p><strong>PARCOURS 0 - ...</strong></p>`, so a
 * parser keyed on `<strong>` would have passed against the real file and failed
 * against the fixture - a test exercising a different code path than production,
 * which is the false-witness family in CLAUDE.md. Detection therefore runs over
 * each block's plain text (`^PARCOURS n -`, `^MODULE n -`, the clock marker) and
 * the raw HTML is kept only to assemble the lesson body.
 *
 * Verified against the real source before this was written: 265 top-level
 * blocks, 100% of the bytes inside one, and text detection yielding exactly 4
 * PARCOURS and 33 MODULE blocks.
 */
 ...
 *
 * The brief named `[EMPLACEMENT VIDEO]`. The document writes
 * `[EMPLACEMENT VIDÉO]`, accented, and there are zero occurrences of the
 * unaccented spelling - a loader searching for the brief's literal would have
 * found none and silently produced no video slots at all. `DOCUMENT` is a third
 * family the brief does not mention; six of them exist.
 */
...
/**
 * Lessons are HTML, and the casing is not cosmetic.
 *
 * `prisma/seed.ts` writes `'video'`, `'pdf'`, `'html'` in lower case. Every
 * reader compares upper case: `courses.service.ts:401` and `:443` gate the
 * signed URL on `contentType === 'VIDEO' || 'PDF'`, the DTO enum is
 * `['VIDEO','PDF','HTML','TEXT']`, and `kbs-lesson-view.tsx` tests `'HTML'`
 * before rendering a body. So every seeded lesson on dev matches no branch and
 * the view falls through to a single dash - the body never renders. This loader
 * writes the casing the readers expect.
 */
