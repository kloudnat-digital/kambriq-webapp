...
export class LandsAdminController {
  ...
  @ApiOperation({
    ...
    description:
      'Creates a new land record. Slug is auto-generated from title if not provided.',
  })
  ...
  @ApiQuery({ name: 'status', required: false, enum: ['AVAILABLE', 'RESERVED', 'SOLD', 'ARCHIVED'], description: 'Filter by land status' })
  @ApiQuery({ name: 'search', required: false, type: String, description: 'Search by title or title number' })
  async listLands(
  ...
  @ApiOperation({
    ...
    description:
      'Soft-delete: sets status to ARCHIVED and unpublishes. Does not remove from DB.',
  })
  @ApiParam({ name: 'id', description: 'Land ID' })
  ...
  @ApiQuery({ name: 'agentUserId', required: false, type: String, description: 'Filter by agent user ID' })
  ...
  @ApiOperation({
    ...
    description:
      'Marks the 5% down payment as received. Reservation becomes CONFIRMED.',
  })
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Complete a sale (finalize reservation)',
    description:
